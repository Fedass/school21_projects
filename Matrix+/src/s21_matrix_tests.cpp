...
TEST_F(S21MatrixTest, OperatorSum1) {
  ...
  expected(2, 1) = -4;
  expected(2, 2) = -6;
  EXPECT_EQ(0, (matrixA + matrixA) == matrixA * 2);
}
...
TEST_F(S21MatrixTest, OperatorSub1) {
  S21Matrix matrixA(*matrix_3x3), expected;
  EXPECT_EQ(0, (matrixA - matrixA) == expected);
}
...
TEST_F(S21MatrixTest, EqMatrix1) {
  ...
  EXPECT_EQ(0, matrixA.EqMatrix(matrixB));
}

TEST_F(S21MatrixTest, EqMatrix2) {
  S21Matrix matrixA, matrixB;
  matrixB(1, 1) = 1;
  EXPECT_EQ(1, matrixA.EqMatrix(matrixB));
}
...
TEST_F(S21MatrixTest, Transpose) {
  ...
  EXPECT_EQ(0, matrixA.Transpose() == expected);
}
...
TEST_F(S21MatrixTest, CalcComplements1) {
  ...
  expected(2, 1) = 34;
  expected(2, 2) = -24;
  EXPECT_EQ(0, matrixA.CalcComplements() == expected);
}
...
TEST_F(S21MatrixTest, Inverse1) {
  ...
  expected(2, 1) = -29;
  expected(2, 2) = 24;
  EXPECT_EQ(0, matrixA.InverseMatrix() == expected);
}
